...
const restoreFile=document.getElementById('db-restore-file')
const restoreBtn=document.getElementById('db-restore')
const restoreStatus=document.getElementById('db-restore-status')
if(backupBtn)backupBtn.addEventListener('click',async()=>{try{if(restoreStatus)restoreStatus.textContent='Creating backup...';const r=await fetch('/api/backup');if(!r.ok){const j=await r.json().catch(()=>({}));if(restoreStatus)restoreStatus.textContent='Backup error: '+(j.error||r.status);return}const disp=r.headers.get('Content-Disposition')||'';const m=/filename=\"?([^\";]+)\"?/i.exec(disp);const name=m?m[1]:'backup.sql';const blob=await r.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();setTimeout(()=>{URL.revokeObjectURL(url);try{document.body.removeChild(a)}catch{}},1000);if(restoreStatus)restoreStatus.textContent='Backup downloaded'}catch(e){if(restoreStatus)restoreStatus.textContent='Backup error: '+(e&&e.message||e)}})
if(restoreBtn)restoreBtn.addEventListener('click',async()=>{if(!restoreFile||!restoreFile.files||!restoreFile.files[0]){if(restoreStatus)restoreStatus.textContent='Choose a .sql file';return}const text=await restoreFile.files[0].text();if(restoreStatus)restoreStatus.textContent='Restoring...';const r=await fetch('/api/restore',{method:'POST',headers:{'Content-Type':'text/plain'},body:text});const j=await r.json().catch(()=>({}));if(restoreStatus)restoreStatus.textContent=r.ok?'Restore completed':('Restore error: '+(j.error||r.status))})
